...
- Docker (shared dev Postgres and DbGate via `./scripts/dev.sh`)
...
| Script                                                | Description                                                                                                                                                                                                                                                                                                                                             |
| ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`scripts/dev.sh`](../scripts/dev.sh)                 | Start shared Postgres + DbGate, then open xfce4-terminal tabs for JWT, DB, Sync, and DOM (`init.sh` then `make dev` in each). Use `--down` to stop Postgres and DbGate.                                                                                                                                                                                 |
...
| [`scripts/init.sh`](../scripts/init.sh)               | Copies example env/config files when missing, starts required Docker services (Postgres, Valkey, DbGate), creates Postgres databases from `.env` / `.env.test` when missing, and runs `make install` or `make dev-install` for one repo (`--repo db`, `--cwd /path/to/NetworthDB`, or `--dev` for the full dev stack). Used by `dev.sh` and `check.sh`. |
